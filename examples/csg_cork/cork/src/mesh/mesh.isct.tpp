...
private:
    // support integration with gong...
    Store newGongStore();
    bool verifyGongIsctResults();
    bool useGongToFindIntersections();
...
template<class VertData, class TriData>
Store Mesh<VertData,TriData>::IsctProblem::newGongStore()
{
    Store gongctxt = Store::NewStore();
    // load data and initialize globals...

    // Load the data into the gong store
    {
        uint N_VERTEX = TopoCache::verts.size();
        std::vector<Vec3d> v_coords(N_VERTEX);
        TopoCache::verts.for_each([&](Vptr v) {
            uint vid = v->ref;
            if (vid >= N_VERTEX) {
                std::cerr << "id out of range(" << N_VERTEX << "): "
                                                << vid << std::endl;
                exit(1);
            }
            v_coords[vid] = vPos(v);
        });
        gongctxt.Vertices().beginload(N_VERTEX);
        gongctxt.Vertices().pos().load( (tensor_double_3_*)(v_coords.data()),
                                        sizeof(Vec3d) );
        gongctxt.Vertices().endload();

        gongctxt.Edges().beginload(TopoCache::edges.size());
        TopoCache::edges.for_each([&](Eptr e) {
            uint v0 = e->verts[0]->ref;
            uint v1 = e->verts[1]->ref;
            gongctxt.Edges().loadrow(v0, v1, (uint64_t)(e));
        });
        gongctxt.Edges().endload();

        gongctxt.Triangles().beginload(TopoCache::tris.size());
        TopoCache::tris.for_each([&](Tptr t) {
            const Tri &tri  = TopoCache::mesh->tris[t->ref];
            auto v          = *(tensor_row_Vertices__3_*)(&tri.v);
            gongctxt.Triangles().loadrow( v, (uint64_t)(t) );
        });
        gongctxt.Triangles().endload();
    }

    // Load the globals into the gong store
    {
        gongctxt.degeneracy_count().write( 0 );
        gongctxt.has_degeneracies().write( false );
        gongctxt.has_intersections().write( false );
        gongctxt.MAGNIFY().write( Quantization::MAGNIFY );
        gongctxt.RESHRINK().write( Quantization::RESHRINK );
    }
...
template<class VertData, class TriData>
bool Mesh<VertData,TriData>::IsctProblem::verifyGongIsctResults() {
    Store gongctxt = newGongStore();
    ...
    uint32_t *et_tri    = gongctxt.ET_Iscts().tri().read_lock();
    uint32_t *et_edge   = gongctxt.ET_Iscts().edge().read_lock();
    Tptr *tids      = (Tptr*)(gongctxt.Triangles().id().read_lock());
    Eptr *eids      = (Eptr*)(gongctxt.Edges().id().read_lock());

    std::cout << "#edges:     "
              << gongctxt.Edges().size() << std::endl;
    std::cout << "#triangles: "
              << gongctxt.Triangles().size() << std::endl;
    std::cout << "found " << gongctxt.ET_Iscts().size()
              << " intersections with gong." << std::endl;
    std::cout << "found " << glue_pts.size()
              << " intersections the old way." << std::endl;
    std::cout << "degen count: "
    ...
    for( uint32_t i=0; i<N_ISCT; i++ ) {
        Eptr    eisct   = eids[et_edge[i]];
        Tptr    tisct   = tids[et_tri[i]];
        Vptr    e0      = eisct->verts[0];
        Vptr    e1      = eisct->verts[1];
        te_cache[tisct->ref].push_back( eisct );

        Tprob prob = reinterpret_cast<Tprob>(tisct->data);
        if(!prob) {
            std::cerr << "could not find CORK intersection(1) of "
                      << "tri:" << tisct->ref
                      << " edge:(" << e0->ref
                            << "," << e1->ref << ")" << std::endl;
            error = true;
            ...
            bool found  = false;
            for( auto iv : prob->iverts ) {
                Eptr e = iv->glue_marker->e;
                if(e == eisct) { found = true; break; }
            }
            if(!found) {
                std::cerr << "could not find CORK intersection(2) of "
                          << "tri:" << tisct->ref
                          << " edge:(" << e0->ref
                                << "," << e1->ref << ")" << std::endl;
                error = true;
    ...
    */
    ...
    //                              << " " << t->verts[2]->ref << std::endl;
        ...
        if (!found) {
            std::cerr << "could not find GONG intersection of "
                      << "tri:" << t->ref
                      << " edge:(" << e0->ref
                            << "," << e1->ref << ")" << std::endl;
            std::cerr << "    "
                      << "edge: " << vPos(e0) << " " << vPos(e1)
                      << std::endl;
            std::cerr << "    "
                      << "tri:  " << vPos(t->verts[0])
                           << " " << vPos(t->verts[1])
    ...
    gongctxt.ET_Iscts().tri().read_unlock();
...
template<class VertData, class TriData>
bool Mesh<VertData,TriData>::IsctProblem::useGongToFindIntersections()
{
    Store gongctxt = newGongStore();
    ...
    bool no_degeneracies = true;
    Empty3d::degeneracy_count = gongctxt.degeneracy_count().read();
    if ( gongctxt.has_degeneracies().read() ) {
        no_degeneracies = false;
    } else { // success, so unpack
        uint32_t N_ISCT     = gongctxt.ET_Iscts().size();
        uint32_t *et_tri    = gongctxt.ET_Iscts().tri().read_lock();
        uint32_t *et_edge   = gongctxt.ET_Iscts().edge().read_lock();
        Tptr *tids      = (Tptr*)(gongctxt.Triangles().id().read_lock());
        ...
        for( uint32_t i=0; i<N_ISCT; i++ ) {
            ...
            IVptr iv = getTprob(tisct)->
                            addInteriorEndpoint(this, eisct, glue);
            for(Tptr tri : eisct->tris) {
                getTprob(tri)->
                            addBoundaryEndpoint(this, tisct, eisct, iv);
            }
        }
...
bool Mesh<VertData,TriData>::IsctProblem::tryToFindIntersections()
{
    ...
    if(use_cork) {
        ...
        std::cout << "+ + hit count cork + " << hitcount << std::endl;
        std::cout << "* * CORK JOIN TIME * "
                  << cork_join_time.stop() << std::endl;
    }

    if (use_gong) {
        Timer total_gong_time;
        if (use_cork) {
            verifyGongIsctResults();
        } else {
            if (!useGongToFindIntersections()) {
                return false;
            }
        }
        std::cout << "* * TOTAL GONG TIME * "
                  << total_gong_time.stop() << std::endl;
    }
